...
function BDPageSuper(){	// shared stuff for BDPage and BDAnnPage
	...
	window.onkeydown = function(e) { 
		if (e.key == "Enter") {
			console.log("Enter key registers");
...
*/
		this.f.action=window.location.protocol+"\\depts.washington.edu/kwatexts/annsaver/index.cgi";
		this.f.method="POST";
...
*/
	src.setAttribute('src', "//depts.washington.edu/kwamedia/"+mediapath+"/wav/0.wav");
	this.ta.appendChild(src);
	this.ta.load();
	var t = document.getElementById("main-container").children[0];
	t.addEventListener('click',function() {this.ta.play();}.bind(this));
	

	// this... did not work
	// sbng need to fix this function to only select space bar
	// need to add left and right arrwo keys
	//t.addEventListener('keydown',this.searchTrack,false);

	//this.searchTrack = function(e) {
		//switch (e.key) {
			// if they pressed space or enter
		//	case 'Enter':
		//	case ' ':
		//	case 'Spacebar':
		//		this.ta.play();
...
*/
										window.location.hostname+"/kwatexts/timestamps/"+
	...
	this.addTSEditors = function(igt){
		...
		for(var i=0;i<wds.length;i++){
			...
			else td.appendChild(document.createTextNode("âˆ…"));
			tr.appendChild(td);
		}
		igt.appendChild(tr);
	}
